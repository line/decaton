...
Decaton supports concurrent processing of tasks in one partition. The reason we need this is described in link:../README.adoc[README] so here I just go through to show how is it effective.
...
If you're using link:https://spring.io/[Spring] for running your applications, you might wanna take a look at link:./integration-spring.adoc[Spring Integration].
