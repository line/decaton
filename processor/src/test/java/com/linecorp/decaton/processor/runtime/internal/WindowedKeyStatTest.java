...
public class WindowedKeyStatTest {
    ...
    public void testRecordAndGet() {
        ...
        assertEquals(new Stat(t0, t0, 1, 1), windowedKeyStat.recordAndGet(t0, key("a")));
        assertEquals(new Stat(t0, t0 + 99, 2, 2), windowedKeyStat.recordAndGet(t0 + 99, key("a")));
        assertEquals(new Stat(t0, t0 + 100, 3, 3), windowedKeyStat.recordAndGet(t0 + 100, key("a")));
        // different key
        assertEquals(new Stat(t0, t0 + 101, 1, 4), windowedKeyStat.recordAndGet(t0 + 101, key("b")));
        // first window (< t0 + 100) is rotated
        assertEquals(new Stat(t0 + 100, t0 + 200, 2, 3), windowedKeyStat.recordAndGet(t0 + 200, key("b")));
    }
