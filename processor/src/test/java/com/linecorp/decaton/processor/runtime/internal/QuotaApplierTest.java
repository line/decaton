...
public class QuotaApplierTest {
    ...
    public void testViolateQuota() throws Exception {
        ...
        TaskRequest request = new TaskRequest(tp, 1, new OffsetState(123), key, null, NoopTrace.INSTANCE,
                                              task.toByteArray(),
                                              new QuotaUsage(UsageType.VIOLATE, metrics));
        doReturn(Action.builder().topic("foo").build()).when(callback).apply(eq(key), eq(metrics));
        assertTrue(applier.apply(request));
        verify(callback, times(1)).apply(eq(key), eq(metrics));
    ...
    public void testComplyQuota() throws Exception {
        TaskRequest request = new TaskRequest(tp, 1, new OffsetState(123), key, null, NoopTrace.INSTANCE,
                                              task.toByteArray(), QuotaUsage.COMPLY);
        assertFalse(applier.apply(request));
